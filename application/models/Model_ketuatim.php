...
class Model_ketuatim extends CI_Model
{
    ...
    function select_pengajuan_search($lpa_id, $tanggal_awal, $tanggal_akhir, $propinsi, $kota, $jenis_fasyankes,$jenis_klinik, $jenis_lab, $status_verifikasi_id)
	{
		...
		if (!empty($jenis_lab) && $jenis_lab == 88) {
			$jenis_lab = " AND data_labkes.jenis_lab='Laboratorium Kesehatan Masyarakat Utama'";
		} else if(!empty($jenis_lab) && $jenis_lab == 77) {
			$jenis_lab = "AND data_labkes.jenis_lab != 'Laboratorium Kesehatan Masyarakat Utama' AND data_labkes.jenis_lab != 'Pengolahan sel punca Utama' AND data_labkes.jenis_lab != 'Bank Jaringan - Yankes'";
		}else{
			$jenis_lab = "";
		}
		...
		$raw_user_id = $jenis_fasyankes2 . $lpa_id . $tanggal_awal . $propinsi2 . $kota . $jenis_klinik . $jenis_lab. $status_verifikasi_id;
		...
		if($jenis_fasyankes==1 || $jenis_fasyankes==null){
			...
			data_klinik.jenis_klinik as jenis_klinik,
			kota.id_kota as id_kota, 
			kota.nama_kota AS nama_kota,";
			$data_join ="LEFT OUTER JOIN dbfaskes.trans_final ON a.fasyankes_id = dbfaskes.trans_final.kode_faskes
