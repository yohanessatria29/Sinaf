...
class Model_ketua extends CI_Model
{
    ...
    function select_pengajuan_search($lpa_id, $tanggal_awal, $tanggal_akhir, $propinsi, $kota, $jenis_fasyankes, $status_verifikasi_id)
	{
		...
		if (!empty($status_verifikasi_id)) {
			if ($status_verifikasi_id == 1) {
				$status_verifikasi_id = " AND p.id IS NULL";
			} else {
				$status_verifikasi_id = "  AND p.id IS NOT NULL";
			}
		} else {
			$status_verifikasi_id = " AND p.id IS NULL";
		}
		...
		$sql = $this->db->query("SELECT
				...
				i.penerimaan_pengajuan_usulan_survei_id,
					...
					p.created_at AS tanggal_pengiriman_rekomendasi
			FROM
				...
				INNER JOIN trans_final_ep_verifikator o on o.penetapan_verifikator_id = n.id
